...
if (manifest.runtime.launch?.executable !== '${installRoot}/bin/hermes' || JSON.stringify(manifest.runtime.launch.args) !== JSON.stringify(['acp']) || manifest.runtime.launch.publishUserCommand !== true) throw new Error('Hermes managed launch contract changed');
...
if (JSON.stringify(candidate.version) !== JSON.stringify({ args: ['--version'], constraint: '>=0.19.0 <0.20.0' })) throw new Error('Hermes discovery version contract changed');
...
const expectedCapabilities = { imageInput: true, audioInput: false, embeddedContext: false, interrupt: true, resume: true, permissionModes: true, modelSelection: false, commands: true, browserUse: true, computerUse: true, skills: true };
